...
DEMO_USERS = {
    "provider1": {
        ...
        "role": "PROVIDER",
        "name": "Dr. John Smith",
        "pw_hash": hash_password("Provider@1234"),
    },
    "reviewer1": {
        "id": "u2",
        "role": "REVIEWER_RN",
        "name": "Sarah Parker RN",
        "pw_hash": hash_password("Review@1234"),
    },
    "meddir1": {
        "id": "u3",
        "role": "MEDICAL_DIRECTOR",
        "name": "Dr. Robert Chen",
        "pw_hash": hash_password("Doctor@1234"),
    },
    "admin": {
        "id": "u4",
        "role": "SUPER_ADMIN",
    ...
    "member1": {
        ...
        "role": "MEMBER",
        "name": "Sarah Johnson",
        "pw_hash": hash_password("Member@1234"),
    },
}
...
async def login(body: LoginRequest, request: Request):
    ...
    if not user or not verify_password(body.password, user["pw_hash"]):
        log.warning(
            "auth.login_failed", username=body.username, ip=get_client_ip(request)
        )
        raise HTTPException(
...
async def refresh(body: dict):
    ...
    jti = str(uuid.uuid4())
    new_payload = {
        k: v for k, v in payload.items() if k not in ("exp", "iat", "type", "jti")
    }
    new_payload["jti"] = jti
