...
async def submit_pa(
    ...
    background_tasks.add_task(
        EventPublisher.publish_submission,
    ...
    background_tasks.add_task(
        ...
        phi_accessed=True,
        phi_fields="member_id,diagnosis,procedure,clinical_summary",
    )
...
async def get_review_queue(
    ...
    try:
        ...
        q = (
            q.order_by(PACase.sla_deadline.asc())
            .offset((page - 1) * limit)
            .limit(limit)
        )
        result = await db.execute(q)
    ...
    return [
        {
            ...
            "status": statuses[i % 5],
            "service_description": services[i % 5],
            "submitted_at": (
...
def _mock_pa_detail(pa_number: str) -> dict:
    return {
        ...
        "payer": "UHC",
        "urgency": "ROUTINE",
        "primary_diagnosis_code": "M511",
        "primary_procedure_code": "72148",
        "ai_confidence": 0.784,
    ...
    return [
        {
            ...
            "sla_deadline": (
                datetime.now(timezone.utc) + timedelta(hours=4 + i * 2)
            ).isoformat(),
            "submitted_at": (
                datetime.now(timezone.utc) - timedelta(hours=i * 3)
            ).isoformat(),
