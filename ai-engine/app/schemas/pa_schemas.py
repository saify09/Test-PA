...
class PASubmissionRequest(BaseModel):
    ...
    def primary_diagnosis(self) -> DiagnosisCode | None:
        return next((d for d in self.diagnoses if d.is_primary), self.diagnoses[0] if self.diagnoses else None)
...
class AIRecommendation(BaseModel):
    ...
    pa_number:          str
    confidence_score:   float = Field(..., ge=0.0, le=1.0, description="0-1 approval confidence")
    ...
    supporting_evidence:List[str] = Field(default_factory=list, description="Excerpts from clinical docs")
    missing_info:       List[str] = Field(default_factory=list, description="Gaps blocking approval")
