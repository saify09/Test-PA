...
class NLPExtractor:
    ...
    @classmethod
    async def extract(cls, request: DocumentExtractionRequest, raw_text: str) -> DocumentExtractionResult:
        ...
        extracted_fields = {
            "dates":          dates[:10],
            "conservative_therapy_mentioned": bool(CONSERVATIVE_PATTERN.search(clean_text)),
            "functional_impairment_mentioned": bool(re.search(
                r"(impair|limit|disab|cannot|unable|difficulty|pain level|VAS)", clean_text, re.I)),
            "prior_imaging_mentioned": bool(re.search(
                r"(prior|previous|prior imaging|x-ray|CT scan|MRI)", clean_text, re.I)),
            "specialist_referral": bool(re.search(
                r"(referral|specialist|orthopedic|neurosurg|rheumatol|oncol)", clean_text, re.I)),
            "emergency_indicators": bool(re.search(
                r"(emergency|urgent|immediate|acute|critical|severe)", clean_text, re.I)),
        ...
        log.info("nlp_extractor.complete",
                 doc=request.document_id, pa=request.pa_number,
                 dx=len(diagnoses), meds=len(medications), ms=round(processing_ms))
    ...
    async def ocr_document(cls, s3_key: str) -> str:
        ...
        try:
            ...
            response = textract.detect_document_text(
                Document={"S3Object": {"Bucket": settings.S3_DOCUMENTS_BUCKET, "Name": s3_key}}
            )
    ...
    def _preprocess(text: str) -> str:
        ...
        expansions = {
            r"\bHx\b": "history", r"\bPx\b": "patient", r"\bDx\b": "diagnosis",
            r"\bRx\b": "prescription", r"\bTx\b": "treatment", r"\bPt\b": "patient",
            r"\bS/P\b": "status post", r"\bH/O\b": "history of", r"\bC/O\b": "complains of",
            r"\bw/o\b": "without", r"\bw/\b": "with",
        }
    ...
    def _extract_pain_scale(text: str) -> Optional[str]:
        m = re.search(r"pain\s+(?:level|scale|score|rating)?[:\s]+(\d+)\s*(?:/\s*10)?", text, re.I)
        if m:
    ...
    def _score_extraction(
        diagnoses: List, procedures: List, medications: List,
        labs: Dict, fields: Dict
    ) -> float:
        score = 0.5
        if diagnoses:   score += 0.15
        if procedures:  score += 0.10
        if medications: score += 0.10
        if labs:        score += 0.05
        if fields.get("conservative_therapy_mentioned"): score += 0.05
        if fields.get("duration_of_symptoms"):           score += 0.03
        if fields.get("pain_scale"):                     score += 0.02
