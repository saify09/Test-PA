...
class ClinicalNLPEngine:
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.tokenizers["biobert"] = AutoTokenizer.from_pretrained(
                "dmis-lab/biobert-v1.1", cache_dir=self.cache_dir, local_files_only=True
            )
    ...
    def classify_medical_necessity(
        self, clinical_text: str, procedure_code: str
    ) -> Dict[str, any]:
        """
    ...
    def _calculate_medical_necessity_score(
        self, clinical_text: str, procedure_code: str
    ) -> float:
    ...
    def _generate_reasoning(
        self, clinical_text: str, procedure_code: str, score: float
    ) -> str:
    ...
    def assess_documentation_quality(self, clinical_text: str) -> Dict[str, any]:
        ...
        word_count = len(clinical_text.split())
        has_codes = bool(
            self._extract_icd10_codes(clinical_text)
            or self._extract_cpt_codes(clinical_text)
        )
        ...
        if missing_elements:
            recommendations.append(
                f"Add documentation for: {', '.join(missing_elements)}"
            )
        if word_count < 100:
