...
class ClinicalCriteriaRAG:
    ...
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            embeddings_model, cache_dir=cache_dir
        )
        self.model = AutoModel.from_pretrained(embeddings_model, cache_dir=cache_dir)
        self.model.eval()
        ...
        if self.index_path.exists():
            ...
            logger.warning(
                f"No index found at {index_path}. Call build_index() to create."
            )
    ...
    def build_index(self, guidelines: List[ClinicalGuideline]):
        ...
        tokenized_guidelines = [
            guideline.to_searchable_text().lower().split() for guideline in guidelines
        ]
    ...
    def load_index(self):
        ...
        tokenized_guidelines = [
            g.to_searchable_text().lower().split() for g in self.guidelines
        ]
        self.bm25_index = BM25Okapi(tokenized_guidelines)
    ...
    def retrieve_guidelines(
        ...
        for idx, score in zip(semantic_indices, semantic_scores):
            hybrid_scores[idx] = (
                hybrid_alpha * score + (1 - hybrid_alpha) * bm25_scores[idx]
            )
        ...
        for idx in sorted_indices[: top_k * 2]:
            ...
            if code_match or not (procedure_code or diagnosis_code):
                result = self._create_retrieval_result(
                    guideline, hybrid_scores[idx], query
                )
    ...
    def _create_retrieval_result(
        self, guideline: ClinicalGuideline, score: float, query: str
    ) -> RetrievalResult:
    ...
    def _extract_matched_criteria(
        self, guideline: ClinicalGuideline, query: str
    ) -> List[str]:
    ...
    def generate_decision(
        ...
        rationale = self._generate_rationale(clinical_text, retrieved, patient_history)

        # Determine recommendation
        if confidence >= 0.85 and self._meets_all_criteria(
            clinical_text, best_match.guideline
        ):
        ...
        alternatives = (
            self._find_alternatives(retrieved[1:]) if len(retrieved) > 1 else []
        )
    ...
    def _generate_rationale(
        ...
        has_symptoms = any(
            word in clinical_text.lower() for word in ["pain", "symptom", "complaint"]
        )
    ...
    def _meets_all_criteria(
        self, clinical_text: str, guideline: ClinicalGuideline
    ) -> bool:
    ...
    def _find_alternatives(self, retrieved: List[RetrievalResult]) -> List[str]:
    ...
    def _handle_no_guidelines_found(
        self, procedure_code: str, diagnosis_code: str
    ) -> RAGDecision:
        ...
        return RAGDecision(
            ...
            missing_documentation=[
                "Clinical guidelines for this procedure/diagnosis combination"
            ],
...
def get_rag_engine(
    ...
    if _rag_engine is None:
        _rag_engine = ClinicalCriteriaRAG(
            guidelines_path=guidelines_path, cache_dir=cache_dir, index_path=index_path
        )
    return _rag_engine
...
if __name__ == "__main__":
    ...
    results = rag.retrieve_guidelines(
        query=clinical_text, procedure_code="72148", diagnosis_code="M54.5"
    )
