...
def route_to_independent_reviewer(
    pa_number: str, original_reviewer_id: Optional[str]
) -> str:
    """
...
async def submit_appeal(req: AppealSubmitRequest, background_tasks: BackgroundTasks):
    ...
    appeal_number = (
        f"APP-{datetime.now().year}-{''.join(random.choices(string.digits, k=6))}"
    )
...
async def update_appeal(
    ...
    if update.decision == AppealDecision.UPHELD and not update.md_cosign_id:
        raise HTTPException(
            422, "Medical Director co-signature required for upheld denial"
        )

    if update.status:
        appeal["status"] = update.status.value
    if update.reviewer_id:
        appeal["reviewer_id"] = update.reviewer_id
    if update.reviewer_notes:
        appeal["reviewer_notes"] = update.reviewer_notes
    if update.decision:
    ...
    if update.decision:
        background_tasks.add_task(
            ...
            appeal_number,
            "APPEAL_DECIDED",
            {"decision": update.decision.value, "overturned": appeal["overturned"]},
        )
    return appeal
...
async def escalate_appeal(
    ...
    background_tasks.add_task(
        publish_appeal_event, appeal_number, "APPEAL_ESCALATED", {"reason": reason}
    )
...
async def sla_at_risk(warning_hours: float = Query(48.0)):
    ...
    for appeal in _appeals.values():
        remaining = days_remaining(appeal["regulatory_deadline"])
        if remaining * 24 <= warning_hours and appeal["status"] not in (
            "DECIDED",
            "CLOSED",
            "WITHDRAWN",
        ):
...
def _seed_mock_appeals():
    for i in range(15):
        ...
        _appeals[num] = {
            ...
            "decision": (
                AppealDecision.OVERTURNED.value
                if (s == AppealStatus.DECIDED and i % 3 == 0)
                else (
                    AppealDecision.UPHELD.value
                    if (s == AppealStatus.DECIDED and i % 3 == 1)
                    else None
                )
            ),
            "overturned": (
                (s == AppealStatus.DECIDED and i % 3 == 0)
                if s == AppealStatus.DECIDED
                else None
            ),
            "submitted_at": (
            ...
            "decided_at": (
                (datetime.now(timezone.utc) - timedelta(days=i // 2)).isoformat()
                if s == AppealStatus.DECIDED
                else None
            ),
        }
