...
# SC-006: 90-day JWT key rotation (HIPAA §164.312)
PRIMARY_SECRET_KEY   = os.getenv("PRIMARY_SECRET_KEY",   "")
SECONDARY_SECRET_KEY = os.getenv("SECONDARY_SECRET_KEY", "")
KEY_VERSION          = int(os.getenv("KEY_VERSION", "1"))

def _active_signing_key() -> str:
    return PRIMARY_SECRET_KEY or SECRET_KEY

def _verification_keys() -> list:
    """All valid verification keys during rotation window."""
    keys = [_active_signing_key()]
    if SECONDARY_SECRET_KEY and SECONDARY_SECRET_KEY != _active_signing_key():
        keys.append(SECONDARY_SECRET_KEY)
    return keys
JWT_ALGORITHM    = "HS256"
...
def _verify_jwt(token: str) -> Optional[dict]:
    """Verify JWT across all rotation keys (SC-006 zero-downtime rotation)."""
    from jose import jwt as _jwt, JWTError
    for key in _verification_keys():
        try:
            return _jwt.decode(token, key, algorithms=[JWT_ALGORITHM])
        except JWTError:
            continue
    try:  # base64 dev fallback
        try:
            import json, base64
...
# SC-001 / NFR-101: Roles that REQUIRE MFA — reviewer, MD, and admin roles
# cannot receive a full access token without providing a valid TOTP code.
MFA_REQUIRED_ROLES: frozenset = frozenset({
    "RN_REVIEWER", "MEDICAL_DIRECTOR", "SUPER_ADMIN", "OPS_ADMIN",
})

# Dev/demo TOTP secrets (base32, 20 bytes). In production these are stored
# encrypted in the user_mfa_credentials table (migration 002) and never logged.
# Any TOTP app (Google Authenticator, Authy) can import these for testing.
_DEMO_TOTP_SECRETS: dict = {
    "reviewer1": "JBSWY3DPEHPK3PXP",  # totp: 6-digit from this secret
    "meddir1":   "JBSWY3DPEHPK3PXQ",
    "admin":     "JBSWY3DPEHPK3PXR",
    "ops1":      "JBSWY3DPEHPK3PXS",
}

def _verify_totp(username: str, code: str) -> bool:
    """Verify TOTP code against stored secret (dev: in-memory; prod: DB lookup)."""
    secret = _DEMO_TOTP_SECRETS.get(username)
    if not secret:
        return False
    ...
    try:
        ...
        pass
    # Fallback: RFC 6238 stdlib TOTP
    import hmac as _hmac, struct as _struct, time as _time, hashlib as _hashlib
    def _hotp(key_b32: str, counter: int) -> str:
        import base64
        key = base64.b32decode(key_b32.upper() + "=" * (8 - len(key_b32) % 8))
        msg = _struct.pack(">Q", counter)
        digest = _hmac.new(key, msg, _hashlib.sha1).digest()
        offset = digest[-1] & 0x0F
        return str((_struct.unpack(">I", digest[offset:offset+4])[0] & 0x7FFFFFFF) % 1_000_000).zfill(6)
    counter = int(_time.time()) // 30
    return any(_hotp(secret, counter + d) == code for d in (-1, 0, 1))
...
DEMO_USERS = {
    ...
    "reviewer1":  {"id": "u-002", "full_name": "Sarah Parker RN", "email": "reviewer1@hospital.org",
                   ...
                   # SC-001: reviewer roles require MFA (mfa_enabled=True)
                   "mfa_enabled": True, "permissions": ["pa:review","pa:approve","pa:deny","pa:pend"]},
    "meddir1":    {"id": "u-003", "full_name": "Dr. Emily Chen MD", "email": "meddir1@hospital.org",
                   ...
                   # SC-001: medical director requires MFA — all denial co-signs are PHI access
                   "mfa_enabled": True, "permissions": ["pa:review","pa:approve","pa:deny","pa:cosign","pa:override"]},
    ...
    "admin":      {"id": "u-005", "full_name": "System Administrator", "email": "admin@hospital.org",
                   ...
                   # SC-001: admin has full system access — MFA mandatory
                   "mfa_enabled": True, "permissions": ["*"]},
    "ops1":       {"id": "u-006", "full_name": "Operations Admin", "email": "ops1@hospital.org",
                   "role": "OPS_ADMIN", "password_hash": _hash_password("Ops@1234"),
                   "mfa_enabled": True, "permissions": ["system:monitor","user:manage","report:view"]},
}
...
async def login(request: LoginRequest, background: BackgroundTasks, req: Request):
    ...
    # SC-001 / NFR-101: MFA enforcement
    # Privileged roles (reviewer, MD, admin) always require MFA — even if
    # mfa_enabled flag is unset on the account (defence-in-depth).
    role = user.get("role", "")
    role_requires_mfa = role in MFA_REQUIRED_ROLES or user.get("mfa_enabled", False)

    if role_requires_mfa and not request.mfa_code:
        raise HTTPException(
            status_code=401,
            detail={
                "error": "MFA_REQUIRED",
                "message": "MFA code required for this role. "
                           "Open your authenticator app and enter the 6-digit code.",
                "mfa_required": True,
            },
        )

    if role_requires_mfa and request.mfa_code:
        # Enforce TOTP verification — invalid code is an auth failure, not a hint
        if not _verify_totp(request.username, request.mfa_code):
            log.warning("auth.mfa_failed", username=request.username,
                        ip=req.client.host if req.client else "unknown")
