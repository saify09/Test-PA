...
async def get_oauth_token(payer: PayerCode) -> str:
    ...
    cfg = {
        PayerCode.UHC: (
            settings.UHC_TOKEN_URL,
            settings.UHC_CLIENT_ID,
            settings.UHC_CLIENT_SECRET,
        ),
        PayerCode.AETNA: (
            f"{settings.AETNA_API_BASE}/oauth/token",
            settings.AETNA_CLIENT_ID,
            settings.AETNA_CLIENT_SECRET,
        ),
        PayerCode.BCBS: (
            f"{settings.BCBS_API_BASE}/oauth/token",
            settings.BCBS_CLIENT_ID,
            settings.BCBS_CLIENT_SECRET,
        ),
        PayerCode.CIGNA: (
            f"{settings.CIGNA_API_BASE}/oauth/token",
            settings.CIGNA_CLIENT_ID,
            settings.CIGNA_CLIENT_SECRET,
        ),
    }.get(payer)
    ...
    async def post(self, path: str, payload: Dict) -> Dict:
        ...
        try:
            async with httpx.AsyncClient(
                timeout=settings.PAYER_TIMEOUT_SECONDS
            ) as client:
                r = await client.post(
            ...
            raise HTTPException(
                status_code=504, detail=f"{self.payer.value} API timeout"
            )
        except httpx.HTTPStatusError as e:
            log.error(
                "payer.api_error", payer=self.payer.value, status=e.response.status_code
            )
            raise HTTPException(
                status_code=502,
                detail=f"{self.payer.value} API error: {e.response.status_code}",
            )
        except Exception as e:
...
def _parse_eligibility_response(
    raw: Dict, req: EligibilityRequest, now: datetime
) -> EligibilityResponse:
...
async def _submit_rest_api(
    req: PASubmissionRequest, now: datetime
    ...
    return PASubmissionResponse(
        pa_number=req.pa_number,
        payer=req.payer.value,
        payer_ref_number=payer_ref,
        status="PENDING",
        turnaround_hours={"ROUTINE": 72, "URGENT": 24, "EMERGENCY": 24}.get(
            req.urgency, 72
        ),
        submitted_at=now.isoformat(),
        source="MOCK",
...
async def _submit_edi_278(
    req: PASubmissionRequest, now: datetime
) -> PASubmissionResponse:
    """Generate and transmit X12 EDI 278 transaction."""
    edi = _build_edi_278(req)
    log.info(
        "edi278.generated", pa=req.pa_number, payer=req.payer.value, length=len(edi)
    )
...
async def check_formulary(req: FormularyRequest) -> FormularyResponse:
    ...
    return FormularyResponse(
        ...
        step_agents=(
            ["methotrexate 15mg x 3 months", "leflunomide 20mg x 3 months"]
            if is_biologic
            else []
        ),
...
async def poll_payer_status(req: PayerStatusPoll) -> Dict[str, Any]:
    ...
    api_configs = {
        PayerCode.UHC: (
            settings.UHC_API_BASE,
            f"/prior-auth/{req.payer_ref_number}/status",
        ),
    ...
    return {
        "pa_number": req.pa_number,
        "payer_ref": req.payer_ref_number,
        "status": "PENDING",
        "payer": req.payer.value,
...
async def sync_authorization_to_claims(
    req: AuthSyncRequest, background_tasks: BackgroundTasks
):
    """
...
async def _sync_auth_to_claims(req: AuthSyncRequest, claims_ref: str) -> None:
    ...
    CLAIMS_SYSTEM_URL = (
        settings.CLAIMS_SYSTEM_URL if hasattr(settings, "CLAIMS_SYSTEM_URL") else None
    )
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            resp = await client.post(
                f"{CLAIMS_SYSTEM_URL}/authorizations", json=payload
            )
        ...
        try:
            ...
            await producer.send(
                "claims-sync-retry",
                _json.dumps({"req": payload, "ref": claims_ref}).encode(),
            )
            await producer.stop()
        except Exception:
            pass
...
async def get_claims_sync_status(pa_id: str):
    ...
    return {
        "pa_id": pa_id,
        "synced": True,
        "claims_ref": f"CLM-{pa_id[:8].upper()}",
        "synced_at": datetime.utcnow().isoformat(),
    }
