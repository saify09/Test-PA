...
const ForgotPasswordPage: NextPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() && !memberId.trim()) { setError('Please enter your email address or Member ID.'); return; }
    setLoading(true); setError('');
    try { await authApi.forgotPassword(email || memberId); } catch {}
    setSent(true); setLoading(false);
  };

  return (
    <>
      <Head><title>Reset Password | MyHealthPA</title></Head>
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8">
            {sent ? (
              <div className="text-center py-4">
                ...
                <p className="text-sm text-gray-500 mb-6">If we find an account matching your information, you'll receive reset instructions within 5 minutes.</p>
                <Link href="/login">
              ...
              <>
                ...
                <p className="text-sm text-gray-400 mb-6">Enter your email address or Member ID to receive a reset link.</p>
                {error && (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" className="form-input pl-10" placeholder="your@email.com"
                  ...
                  <div>
                    ...
                    <input type="text" className="form-input" placeholder="MB12345678"
                      value={memberId} onChange={e => setMemberId(e.target.value)} />
                    <p className="text-xs text-gray-400 mt-1">Found on your insurance card</p>
                  </div>
                  <button type="submit" disabled={loading}
                    className={cn('w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold text-white text-sm transition-all',
