...
class StorageBackend:
    ...
    async def _store_s3(file_bytes: bytes, key: str) -> str:
        try:
            ...
            s3.put_object(
                Bucket=settings.S3_BUCKET, Key=key, Body=file_bytes,
                ServerSideEncryption="aws:kms",  # AES-256 via KMS
                Metadata={"hipaa": "phi", "uploaded": datetime.now(timezone.utc).isoformat()},
            )
...
def classify_document(filename: str, extracted_text: str) -> DocumentType:
    ...
    patterns = [
        (DocumentType.LAB_RESULTS,      ["lab", "laboratory", "hba1c", "cbc", "bmp", "lipid", "culture"]),
        (DocumentType.IMAGING_REPORT,   ["radiology", "mri", "ct scan", "x-ray", "ultrasound", "impression:", "technique:"]),
        (DocumentType.OPERATIVE_REPORT, ["operative", "procedure:", "surgeon:", "anesthesia", "incision"]),
        (DocumentType.DISCHARGE_SUMMARY,["discharge", "hospital course", "disposition:", "follow-up:"]),
        (DocumentType.PRESCRIPTION,     ["prescription", "sig:", "dispense", "refills:", "rx#", "pharmacy"]),
        (DocumentType.REFERRAL_LETTER,  ["referral", "refer", "specialist", "consultation request"]),
        (DocumentType.PRIOR_AUTH_FORM,  ["prior auth", "authorization request", "pa form", "formulary"]),
        (DocumentType.APPEAL_LETTER,    ["appeal", "reconsideration", "overturn", "grievance"]),
...
def extract_entities(text: str) -> Dict[str, Any]:
    ...
    fields = {
        ...
        "functional_impairment": bool(re.search(
            r"(impair|limit|disab|cannot|unable|difficulty)", text, re.I)),
        ...
        "dates_mentioned":       re.findall(
            r"\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b|\b\d{4}-\d{2}-\d{2}\b", text)[:10],
    }
    return {"diagnoses": diagnoses, "procedures": procedures,
            "medications": medications, "labs": labs, "fields": fields}


def _extract_duration(text: str) -> Optional[str]:
    m = re.search(r"(\d+)\s*(day|week|month|year)s?\s*(of\s*)?(symptom|pain|condition)?", text, re.I)
    return f"{m.group(1)} {m.group(2)}s" if m else None

def _extract_pain(text: str) -> Optional[str]:
    m = re.search(r"pain\s*(?:score|scale|level|rating)?[:\s]+(\d+)\s*(?:/\s*10)?", text, re.I)
    return f"{m.group(1)}/10" if m else None

def _extract_physician(text: str) -> Optional[str]:
    m = re.search(r"(?:dr\.?|doctor|physician|provider)[:\s]+([A-Z][a-z]+ [A-Z][a-z]+)", text, re.I)
...
async def run_ocr(file_bytes: bytes, mime_type: str) -> Tuple[str, float]:
    ...
    if settings.AWS_ACCESS_KEY_ID:
        try:
            ...
            conf  = sum(b.get("Confidence", 95) for b in blocks) / max(len(blocks), 1) / 100
            log.info("ocr.textract_success", blocks=len(blocks), confidence=round(conf, 3))
            return text, conf
        except Exception as e:
...
async def process_document(doc_id: str):
    ...
    try:
        ...
        log.info("document.processed", doc_id=doc_id, type=doc_type.value,
                 dx=len(entities["diagnoses"]), ms=round(processing_ms))
...
@app.get("/health")
async def health():
    return {"status":"healthy","service":"document-processing","version":settings.APP_VERSION}
...
async def upload_document(
    ...
    if not is_clean:
        log.warning("document.virus_detected", filename=file.filename, result=scan_result)
    ...
    log.info("document.uploaded", doc_id=doc_id, pa=pa_number,
             filename=file.filename, size_mb=round(size_mb, 2))
    return {
        "document_id": doc_id, "pa_number": pa_number,
...
async def get_extraction(doc_id: str):
    ...
    if doc["status"] not in (ProcessingStatus.EXTRACTED.value,):
        return {"document_id": doc_id, "status": doc["status"], "message": "Processing not complete"}
    return ExtractionResult(
        document_id=doc_id, pa_number=doc.get("pa_number"),
...
async def download_document(doc_id: str):
    ...
    url = await StorageBackend.presigned_url(doc.get("s3_key",""), settings.PRESIGNED_URL_EXPIRY)
    if url:
...
@app.get("/documents")
async def list_documents(pa_number: Optional[str] = None, page: int = 1, limit: int = 20):
    ...
    start = (page - 1) * limit
    return {
        "items": [{k:v for k,v in d.items() if k != "extracted_text"}
                  for d in docs[start:start+limit]],
        "total": len(docs), "page": page,
    }
